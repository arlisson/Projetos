...
class EdgeDetectionApp:
    def __init__(self, root):
        ...
        # Label para exibir a imagem filtrada com o filtro plus
        self.label_filtered_image_plus = Label(self.root)
        self.label_filtered_image_plus.pack(side='right')

        # Label para exibir a imagem filtrada com o filtro minus
        self.label_filtered_image_minus = Label(self.root)
        self.label_filtered_image_minus.pack(side='right')
    ...
    def apply_plus_filter(self):
        if self.image_rgb1 is not None and self.image_rgb2 is not None:
            ...
            result = cv2.add(self.image_rgb1, self.image_rgb2)
            self.display_image(result, self.label_filtered_image_plus,
                               "Operação Aritmética Adição")
            self.filtered_image_plus = result

    def apply_minus_filter(self):
        if self.image_rgb1 is not None and self.image_rgb2 is not None:
            ...
            # Aplicar a operação AND entre as duas imagens
            result = cv2.subtract(self.image_rgb1, self.image_rgb2)
            self.display_image(result, self.label_filtered_image_minus,
                               "Operação Aritmética Subtração")
            self.filtered_image_and = result

    def apply_all_filters(self):
        # Aplicar todos os filtros
        self.apply_plus_filter()
        self.apply_minus_filter()
