...
def criar_tela_cadastro_produto(app):
    ...
    def criar_entrada(frame, label, row, width=50):
        ttk.Label(frame, text=label).grid(row=row, column=0, sticky="w", padx=5, pady=3)
        entry = ttk.Entry(frame, width=width)
        entry.grid(row=row, column=1, sticky="ew", padx=5, pady=3, columnspan=2)
        return entry
    ...
    campos["data_scraping"] = criar_entrada(form_frame, "Data:", 5)
    ...
    imagem_frame.grid(row=0, column=1, padx=10, pady=10, sticky="n")
    ...
    def salvar():
        try:
            produto = {
                ...
                "data_scraping": campos["data_scraping"].get(),
